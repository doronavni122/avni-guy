---
title: "מתי משתלם לפצל עסקת מכר ולחסוך במס"
description: "פיצול עסקת מכר למיסוי: סעיף 22, תנאי סבירות, סיכוני רשות המיסים ותכנון מס רכישה ושבח יחד."
metaTitle: "גיא אבני עורך דין | פיצול עסקת מכר למיסוי | ישראל | ישראל"
metaDescription: "מתי משתלם לפצל עסקת מכר? סעיף 22, תנאי סבירות וסיכונים. גיא אבני עורך דין מסביר דוגמאות, מס רכישה ושבח לפני חתימה. המדריך מסביר צעדים מעשיים, דוגמאות מהשטח וטעויות נ"
mainKeyword: "גיא אבני עורך דין"
pubDate: "2026-06-01"
category: "tax"
updatedDate: "2026-06-01"
materialChange: true
tags: ["tax", "sale-transaction", "planning"]
internalLinks: ["/", "/about/", "/services/", "/blog/guy-avni-capital-gains-tax-second-apartment/", "/blog/guy-avni-second-apartment-purchase-tax-calculation/", "/blog/guy-avni-purchase-tax-exemption-first-apartment/", "/blog/guy-avni-linear-capital-gains-tax-benefit/", "/blog/", "/categories/tax/", "/tags/sale-transaction/", "/tags/planning/", "/contact/", "/tags/"]
images:
  - src: "https://avniguy.co.il/images/blog/guy-avni-split-sale-transaction-tax-savings-main-guy-avni-lawyer-category-tax-tags-planning-sale-transaction-tax-img-1-photo-1.jpg"
    alt: "תצלום משפטי 1 - guy-avni-lawyer tax tax sale-transaction planning img-1 photo-1 - Avni Guy legal content"
    title: "guy-avni-lawyer tax tax sale-transaction planning img-1 photo-1 | legal image 1 | guy-avni-split-sale-transaction-tax-savings"
    description: "Description 1 (EN keywords): guy-avni-lawyer; tax; tax; sale-transaction; planning; img-1; photo-1. Legal stock photo for blog article guy-avni-split-sale-transaction-tax-savings."
    source: "https://unsplash.com/license"
  - src: "https://avniguy.co.il/images/blog/guy-avni-split-sale-transaction-tax-savings-main-guy-avni-lawyer-category-tax-tags-planning-sale-transaction-tax-img-2-photo-2.jpg"
    alt: "תצלום משפטי 2 - guy-avni-lawyer tax tax sale-transaction planning img-2 photo-2 - Avni Guy legal content"
    title: "guy-avni-lawyer tax tax sale-transaction planning img-2 photo-2 | legal image 2 | guy-avni-split-sale-transaction-tax-savings"
    description: "Description 2 (EN keywords): guy-avni-lawyer; tax; tax; sale-transaction; planning; img-2; photo-2. Legal stock photo for blog article guy-avni-split-sale-transaction-tax-savings."
    source: "https://unsplash.com/license"
  - src: "https://avniguy.co.il/images/blog/guy-avni-split-sale-transaction-tax-savings-main-guy-avni-lawyer-category-tax-tags-planning-sale-transaction-tax-img-3-photo-3.jpg"
    alt: "תצלום משפטי 3 - guy-avni-lawyer tax tax sale-transaction planning img-3 photo-3 - Avni Guy legal content"
    title: "guy-avni-lawyer tax tax sale-transaction planning img-3 photo-3 | legal image 3 | guy-avni-split-sale-transaction-tax-savings"
    description: "Description 3 (EN keywords): guy-avni-lawyer; tax; tax; sale-transaction; planning; img-3; photo-3. Legal stock photo for blog article guy-avni-split-sale-transaction-tax-savings."
    source: "https://unsplash.com/license"
---

## מהו פיצול עסקת מכר במיסוי

פיצול עסקה הוא חלוקת תמורת המכירה למספר רכיבים או שלבים (מגרש ודירה, זכויות בנייה, חניה, שלב תשלום נפרד), כך שכל רכיב ממוסה לפי הכללים החלים עליו. המטרה: ניצול מדרגות, פטורים או דחיית מס כשהדין מאפשר. הבסיס: [חוק מיסוי מקרקעין](https://www.gov.il/he/departments/ministry_of_justice/govil-landing-page), בדגש סעיף 22 וסבירות עסקה. [גיא אבני עורך דין](/) מזהיר: פיצול שלא נתמך בנסיבות עלול להיחשב כעסקה מלאכותית.

לפני תכנון: [רקע המשרד](/about/) ו-[תחומי ליווי](/services/).

## מתי הרשות מאשרת ומתי דוחה

רשות המיסים בוחנת:
- **כלכליות:** האם המחירים משקפים שווי שוק לכל רכיב.
- **זמניות:** האם השלבים מופרדים בזמן באופן אמיתי.
- **תיעוד:** חוזים נפרדים, חשבוניות, שמאות.

פיצול "ניירי" בלבד, בלי הפרדה כלכלית, מסוכן. יש לצרף חוות דעת שמאית כשיש פער משמעותי בין רכיבים.

## דוגמאות מהשטח

**מגרש + בית:** מכירת מגרש וזכויות בנייה בנפרד ממכירת המבנה, כשיש בסיס לשווי שונה. **חניה ומחסן:** לעיתים ניתן לבחון הפרדה אם אינם חובה בהסכם המקורי. **תמורה מדורגת:** תשלומים על פני שנים: כל תשלום עשוי להיחשב אירוע מס בנפרד.

השוו ל-[מס שבח על דירה שנייה](/blog/guy-avni-capital-gains-tax-second-apartment/) ול-[חישוב מס רכישה לדירה נוספת](/blog/guy-avni-second-apartment-purchase-tax-calculation/) כדי לראות את המס בכניסה וביציאה.

## קשר למס רכישה ולדירה יחידה

הרוכש בפיצול עשוי לשלם מס רכישה שונה לכל רכיב (מגרש מול מגורים). המוכר צריך לבדוק שבח לכל אירוע מס. [פטור מס רכישה לדירה ראשונה](/blog/guy-avni-purchase-tax-exemption-first-apartment/) רלוונטי רק כשהרכיב מוגדר כדירת מגורים יחידה.

## לינארי ונכסים ותיקים

במכירת נכס מלפני 2014, [החישוב הלינארי](/blog/guy-avni-linear-capital-gains-tax-benefit/) משפיע על שבח בכל שלב. פיצול לא יוצר פטור שלא קיים בדין.

## סיכונים וליווי מקצועי

- דחייה בשומה או חיוב מלא כעסקה אחת.
- ריבית והצמדה על הפרש.
- עיכוב בטאבו אם יש מחלוקת מס.

לכן: תכנון לפני חתימה, תיעוד, ולעיתים הליך מקדים מול רשות המיסים.

## צעדים מעשיים

1. שמאות מפורטת לרכיבים.
2. ניסוח חוזים נפרדים (אם רלוונטי).
3. סימולציית מס למוכר ולקונה.
4. דיווח מדויק בטפסים.

[מאגר מאמרים](/blog/), [קטגוריית tax](/categories/tax/), [תגית sale-transaction](/tags/sale-transaction/), [תגית planning](/tags/planning/), [יצירת קשר](/contact/), [תגיות](/tags/).

## שאלות נפוצות

**האם כל מוכר יכול לפצל?** רק כשיש בסיס עסקי ומשפטי; לא תכנון מס בלבד.

**האם פיצול חוסך תמיד?** לא; לעיתים המס הכולל דומה או גבוה יותר בגלל אובדן פטורים.

**מה עם קבוצת רכישה?** כל חבר נבחן בנפרד; פיצול פנימי מורכב יותר.

**האם צריך אישור מראש?** לא חובה, אך בקשה מקדימה מפחיתה סיכון.

מקורות רשמיים: [רשות המיסים בישראל](https://www.gov.il/he/departments/israel_tax_authority/govil-landing-page) ו-[לשכת עורכי הדין](https://www.israelbar.org.il/). המידע במאמר הוא לצורכי הסברה כללית ואינו מהווה ייעוץ מס או משפטי אישי.
## סעיף 22 בקצרה

סעיף 22 לחוק מיסוי מקרקעין עוסק בעסקאות בין קשורים ובהיבטי סבירות. פיצול בין זרים לא מבטל בדיקה: הרשות שואלת אם העסקה אמיתית. תיעוד מחירים לרכיבים שונה, חוזים נפרדים, ותשלומים בנפרד מחזקים עמדה.

## דוגמה מספרית מפושטת

מכירה 3,000,000: דירה 2,200,000 + חניה 400,000 + מחסן 400,000. אם שווי השוק של החניה 250,000, הרשות עלולה לשבץ מחיר שוק ולחייב מס על ההפרש. שמאות מפורטת מקדימה מפחיתה סיכון.

## שאלות נפוצות נוספות

**האם פיצול עובד במתנה?** כללי מתנה שונים; לא תמיד זהה למכירה.

**מה עם מס שבח על כל שלב?** כל אירוע מס נבחן בנפרד.

**האם עורך דין חייב?** לא חובה, אך מומלץ בסכומים גבוהים.

## פיצול מול מתנה מול חברה

פיצול במכירה לצד שלישי שונה ממתנה בין קרובים: במתנה יש כללי מס שונים ולעיתים פטורים מוגבלים. מכירה דרך חברה בע"מ מוסיפה שכבת מס חברות ומס שבח למשקיע. כל מבנה דורש ניתוח נפרד; אין פתרון אחד לכולם.

## תיאום עם קונה ועורכי דין

הקונה חייב להבין איך מחולקת התמורה: מס רכישה שונה לרכיבים. חוזה מכר צריך לשקף את הפיצול בפועל, לא רק בנספח מס. עורך דין לצד המוכר ולצד הקונה מונע מחלוקת בטאבו.

## ביקורת רטרואקטיבית

הרשות רשאית לבחון עסקאות שנים אחורה. פיצול שלא נתמך עלול להוביל לשומה מחדש וריבית. תיעוד שמאי וכלכלי חשוב גם שנים לאחר העסקה.

## פיצול בין דירה, חניה ומחסן: דוגמה מעשית

בעסקה אחת של 2,800,000 שקלים, המוכר מבקש לשייך 200,000 לחניה ו-150,000 למחסן. הרשות תבדוק האם המחירים עולים בקנה אחד עם שמאות שוק. אם החניה שווה 120,000, ייתכן שיושב שווי שוק לחניה והשאר ייחשב חלק מהדירה. התוצאה משפיעה על מס רכישה לקונה ועל שבח למוכר.

## מתי לא כדאי לפצל

כשאין הפרדה כלכלית אמיתית, כשהעלות המשפטית והשמאית גבוהה מהחיסכון, או כשהעסקה קטנה. במקרים אלה, עסקה אחת שקופה עדיפה. ייעוץ מס קצר לפני חתימה מחזיר את ההחלטה להיגיון כלכלי.

## עסקאות בין קשורים ומחיר לא שוק

מכירה בין הורים לילדים או בין שותפים מעוררת בדיקה. פיצול לא יפחית את החשד אם המחיר הכולל נמוך. יש להציג נימוק כלכלי: מצב הנכס, זכויות בנייה, או מגבלות שימוש. שקיפות מול הרשות עדיפה על "אופטימיזציה" אגרסיבית.

## סיכום לפני יישום

שמאות לרכיבים, חוזים תואמים, סימולציית מס למוכר ולקונה, ובדיקת סעיף 22. כך מקטינים סיכון לשומה מלאכותית ועיכוב בטאבו.


בעסקאות גדולות, כדאי לבחון פנייה מקדימה לרשות המיסים עם תיאור המבנה והשמאות. זה לא מבטיח אישור, אך מפחית הפתעה. המוכר והקונה צריכים להבין ששומה עתידית עלולה לשנות את חלוק המס ביניהם אם לא נקבע בחוזה מי נושא בסיכון שומה.

גיא אבני עורך דין מדגיש: פיצול עסקה הוא כלי תכנון, לא טריק אוטומטי. בלי ניתוח מספרי לשני הצדדים, אין להמליץ על מבנה. שמירת מסמכי שמאות וחוזים במקום אחד מקלה על ביקורת עתידית.

בעסקאות עם חניה ומחסן, בדקו האם הרכיבים רשומים בנפרד בטאבו. אם לא, פיצול במחיר בלבד חלש יותר. לעיתים רישום נפרד לפני מכירה מחזק את העמדה.

לפני חתימה, בקשו חוות דעת מס שמפרטת את המבנה המוצע, את הסיכונים, ואת ההשוואה לעסקה אחת. כך ניתן להציג לרשות נימוק מסודר אם תתעורר ביקורת. גיא אבני עורך דין ממליץ לשמור את החוות דעת עם החוזים.

בסוף התהליך, מומלץ לעבור על טופסי המס של שני הצדדים יחד עם יועץ, כדי לוודא שהפיצול בחוזה תואם את הדיווח לרשות.

גיא אבני עורך דין מסכם: פיצול עסקה דורש שקיפות מול שני הצדדים, שמאות עדכנית, ונכונות לשלם מס מלא אם הרשות תדחה את המבנה.
