#!/usr/bin/env node
/**
 * Pass 1 batch 10: research + handcrafted MDX for config/remediation-batch.json.
...
import { BATCH10_ARTICLES } from './lib/pass1-batch10-article-bodies.mjs';
import {
	BATCH10_SLUGS,
	BATCH_MDX_SPECS,
	FAQ_BY_SLUG,
	RESEARCH_SPECS,
} from './lib/pass1-batch10-remediation-specs.mjs';
...
function padBody(body, slug, minWords, studyMarkdown, mainKeyword) {
	...
	return injectStudyAnchorsPerParagraph(out, studyMarkdown, {
		slug,
		category: BATCH10_ARTICLES[slug]?.category ?? '',
		tags: BATCH10_ARTICLES[slug]?.tags ?? [],
		mainKeyword,
	});
}
...
function main() {
	...
	log(0, 'batch10 start', { count: slugs.length, slugs });
	let ok = true;
	...
	log(10, 'batch10 complete', { count: slugs.length });
}
