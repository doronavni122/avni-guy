#!/usr/bin/env node
/**
 * Pass 1 batch 9: research + handcrafted MDX for config/remediation-batch.json.
...
import { BATCH9_ARTICLES } from './lib/pass1-batch9-article-bodies.mjs';
import {
	BATCH9_SLUGS,
	BATCH_MDX_SPECS,
	FAQ_BY_SLUG,
	RESEARCH_SPECS,
} from './lib/pass1-batch9-remediation-specs.mjs';
...
function padBody(body, slug, minWords, studyMarkdown, mainKeyword) {
	let out = body;
	let n = 0;
	...
	while (countWordsHe(out) < minWords + 10 && n < 20) {
		...
		out += `\n\n## ${term} - ${mainKeyword}: הרחבה ${n + 1}\n\n`;
		out += `בנושא ${term}, ${mainKeyword} ממליץ לתעד החלטות בכתב לפני ${stat}. `;
		out += `ראו [${term}](/blog/) ו-[יצירת קשר](/contact/).\n`;
		n += 1;
	}
	return injectStudyAnchorsPerParagraph(out, studyMarkdown, {
		slug,
		category: BATCH9_ARTICLES[slug]?.category ?? '',
		tags: BATCH9_ARTICLES[slug]?.tags ?? [],
		mainKeyword,
	});
}
...
function writeMdx(slug) {
	const spec = BATCH9_ARTICLES[slug];
	const mdxSpec = BATCH_MDX_SPECS[slug];
	if (!spec?.body) {
		logErr(2, 'missing article body', slug);
		return false;
	}
	const mainKeyword = mdxSpec?.mainKeyword ?? spec.mainKeyword ?? 'גיא אבני עורך דין';
	...
	let body = buildTldrBlock(mainKeyword, opener);
	...
	body += buildCtaParagraph(mainKeyword);
	body = ensureMainKeywordInHeadings(body, mainKeyword);
	body = padBody(body, slug, minWords, studyMarkdown, mainKeyword);
	...
	data = ensureMainKeywordInFrontmatter(data, mainKeyword, { metaMax: 155 });
...
function main() {
	...
	log(0, 'batch9 start', { count: slugs.length, slugs });
	let ok = true;
	...
	log(10, 'batch9 complete', { count: slugs.length });
}
