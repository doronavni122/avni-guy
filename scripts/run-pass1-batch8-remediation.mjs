#!/usr/bin/env node
/**
 * Pass 1 batch 8: research + handcrafted MDX for config/remediation-batch.json.
...
import { BATCH8_ARTICLES } from './lib/pass1-batch8-article-bodies.mjs';
import {
	BATCH8_SLUGS,
	BATCH_MDX_SPECS,
	FAQ_BY_SLUG,
	RESEARCH_SPECS,
} from './lib/pass1-batch8-remediation-specs.mjs';
...
function padBody(body, slug, minWords, studyMarkdown, mainKeyword) {
	...
	return injectStudyAnchorsPerParagraph(out, studyMarkdown, {
		slug,
		category: BATCH8_ARTICLES[slug]?.category ?? '',
		tags: BATCH8_ARTICLES[slug]?.tags ?? [],
		mainKeyword,
	});
}
...
function writeMdx(slug) {
	const spec = BATCH8_ARTICLES[slug];
...
function main() {
	...
	log(0, 'batch8 start', { count: slugs.length, slugs });
	let ok = true;
	...
	log(10, 'batch8 complete', { count: slugs.length });
}
